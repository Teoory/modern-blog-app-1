...
const Header = () => {
    ...
    useEffect(() => {
        const checkNewNotifications = async () => {
            try {
                const response = await fetch(`http://https://fiyaskoblog-api.vercel.app/check-new-notifications?userId=${userInfo.id}`);
                if (response.ok) {
    ...
    function logout() {
        fetch('http://https://fiyaskoblog-api.vercel.app/logout', {
          credentials: 'include',
          method: 'POST',
        }).then(() => {
          setUserInfo(null);
        ...
        <>
            ...
            <div className="login-button">
                <div className="dropdown">
                    ...
                    {profilePhoto ? (
                        <img className='ProfilePhoto' src={`http://https://fiyaskoblog-api.vercel.app/${profilePhoto}`} alt="Profile" />
                    ) : (
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
