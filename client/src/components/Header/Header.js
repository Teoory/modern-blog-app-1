...
const Header = () => {
    ...
    useEffect(() => {
        const checkNewNotifications = async () => {
            try {
                const response = await fetch(`https://fiyaskoblog-api.vercel.app/check-new-notifications?userId=${userInfo.id}`);
                if (response.ok) {
        ...
        <>
            ...
            <div className="login-button">
                <div className="dropdown">
                    ...
                    {profilePhoto ? (
                        <img className='ProfilePhoto' src={`https://fiyaskoblog-api.vercel.app/${profilePhoto}`} alt="Profile" />
                    ) : (
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
