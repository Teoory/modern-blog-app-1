...
const UserProfilePage = () => {
  ...
  useEffect(() => {
    fetch(`http://https://fiyaskoblog-api.vercel.app/profile/${username}`)
  ...
  fetch('http://https://fiyaskoblog-api.vercel.app/profilephoto', {
      credentials: 'include',
    })
      .then(response => response.json())
      .then(data => {
  ...
  useEffect(() => {
      ...
      fetch(`http://https://fiyaskoblog-api.vercel.app/profile/${username}/likedPosts`)
    .then(response => response.json())
    .then(data => {
  ...
  async function newProfilePhoto(ev) {
    ...
    const response = await fetch('http://https://fiyaskoblog-api.vercel.app/profilePhoto', {
      method: 'POST',
      body: data,
      credentials: 'include',
    });
                ...
                {userInfo.username === username ?
                    <form onSubmit={newProfilePhoto} id="profilePhotoForm">
                      <div className="ppContent">
                        ...
                        {profilePhoto && (
                          <img src={`http://https://fiyaskoblog-api.vercel.app/${user.profilePhoto}`} alt="Profile" />
                          )}
                        </div>
                        {!isFileSelected 
                        ? null
              ...
              <div className='LastPostImage'>
                  ...
                  <div key={post._id} className="LastPostImageOverlay">
                      <Link to={`/post/${post._id}`} className='BlogTitle'>
                          <img src={'http://https://fiyaskoblog-api.vercel.app/'+post.cover} alt="img" />
