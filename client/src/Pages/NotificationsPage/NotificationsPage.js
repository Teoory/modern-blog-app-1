...
const NotificationsPage = () => {
    ...
    useEffect(() => {
        const fetchNotifications = async () => {
            try {
                const response = await fetch(`https://fiyasko-blog-app.vercel.app/notifications/${userInfo.id}`);
                if (response.ok) {
    ...
    useEffect(() => {
        const markAllNotificationsAsRead = async () => {
            try {
                const response = await fetch('https://fiyasko-blog-app.vercel.app/mark-all-notifications-as-read', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
