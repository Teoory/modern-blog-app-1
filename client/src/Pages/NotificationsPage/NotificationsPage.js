...
const NotificationsPage = () => {
    ...
    useEffect(() => {
        const fetchNotifications = async () => {
            try {
                const response = await fetch(`https://modern-blog-app-1.vercel.app/notifications/${userInfo.id}`);
                if (response.ok) {
    ...
    useEffect(() => {
        const markAllNotificationsAsRead = async () => {
            try {
                const response = await fetch('https://modern-blog-app-1.vercel.app/mark-all-notifications-as-read', {
                    method: 'POST',
                    headers: {
