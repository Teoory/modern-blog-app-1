...
const NotificationsPage = () => {
    ...
    useEffect(() => {
        const fetchNotifications = async () => {
            try {
                const response = await fetch(`http://https://fiyaskoblog-api.vercel.app/notifications/${userInfo.id}`);
                if (response.ok) {
    ...
    useEffect(() => {
        const markAllNotificationsAsRead = async () => {
            try {
                const response = await fetch('http://https://fiyaskoblog-api.vercel.app/mark-all-notifications-as-read', {
                    method: 'POST',
                    headers: {
